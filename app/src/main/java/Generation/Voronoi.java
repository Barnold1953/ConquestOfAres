...
public class Voronoi {
    ...
    private static List getAllIntersects(final Segment l, final Segment s) {
        List<CompoundLine> intersects = new LinkedList<CompoundLine>();
        ...
        for (VoronoiSegment ss : v1.edges) {
        ...
        for (VoronoiSegment ss : v2.edges) {
            Segment sss = new Segment();
            sss.p1 = ss.p1;
            sss.p2 = ss.p2;
            sss.i1 = ss.i1;
    ...
    /** find first point */
    private static CompoundLine findFirstPoint(Segment line, final List<CompoundLine> intersects, final Segment s) {
        CompoundLine m;
    ...
    /** find next point */
    private static CompoundLine findNextPoint(Segment line, final List<CompoundLine> intersects, final Segment s) {
        CompoundLine m;
        if(intersects.isEmpty())
            return null;

        Iterator<CompoundLine> iter = intersects.iterator();
        do {
            if(!iter.hasNext())
                return null;
            m = iter.next();
